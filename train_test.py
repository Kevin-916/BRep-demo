...
from model.GCN_DAE import dae_recon_loss
...
def train(model, optimizer, args, train_loader, epoch):
    ...
    for time_series, node_feature, label in train_loader:
        ...
        output, dae_h, dae_noise = model(time_series, node_feature)
        label = label.float()

        optimizer_update(optimizer=optimizer, step=step, total_steps=total_steps, args=args)
        optimizer.zero_grad()
        ce_loss = criterion(output, label)
        dae_loss = dae_recon_loss(time_series,dae_h,dae_noise)
        loss = ce_loss + args.dae_loss_percent * dae_loss
        loss.backward()
        optimizer.step()
        ...
        #         f"DAE: {dae_loss.item():.4f} | "
        #         f"KD: {model.kd_loss().item():.4f}")
